...
indexrs provides a CLI that outputs structured, fzf-friendly data for terminal-based fuzzy searching. This document specifies output formats, integration recipes, preview commands, shell functions, and editor integration.

## Design Principles

- **indexrs is the data source; fzf is the UI.** indexrs outputs lines to stdout with ANSI colors and structured delimiters. fzf handles filtering, selection, and preview orchestration.
...
### 1.1 File Search Mode (`indexrs files`)
...
### 1.2 Content Search Mode (`indexrs search`)
...
```
$ indexrs search 'trigram' --color=always
...
### 1.3 Symbol Search Mode (`indexrs symbols`)
...
```
$ indexrs symbols 'TrigramIndex' --color=always
...
### 1.4 Preview Subcommand (`indexrs preview`)

A dedicated subcommand that renders file content for fzf's preview window. This exists so that indexrs can provide syntax-highlighted, line-centered previews without requiring bat as a dependency (though bat is preferred when available).

```
$ indexrs preview <file> [--line <n>] [--context <n>]
```
...
```bash
indexrs files --color=always | fzf \
  --ansi \
  --scheme=path \
...
The key pattern: fzf starts with `--disabled` (no local filtering), and on every keystroke, reloads results from indexrs. This delegates all searching to the index.

```bash
indexrs search '' --color=always --limit=500 | fzf \
  --ansi \
  --disabled \
  --delimiter : \
  --prompt 'search> ' \
  --header 'CTRL-R: toggle regex | CTRL-F: switch to files | ENTER: open' \
  --bind 'change:reload:indexrs search {q} --color=always --limit=500 || true' \
  ...
  --bind 'ctrl-r:+reload:
    if [[ $FZF_PROMPT == "regex> " ]]; then
      indexrs search {q} --regex --color=always --limit=500 || true
    else
      indexrs search {q} --color=always --limit=500 || true
    fi' \
...
: | fzf \
  ...
  --bind 'start:reload:indexrs search {q} --color=always --limit=500 || true' \
  --bind 'change:reload:indexrs search {q} --color=always --limit=500 || true' \
  --bind 'ctrl-f:unbind(change)+change-prompt(files> )+enable-search+reload(indexrs files --color=always)' \
  --bind 'ctrl-g:rebind(change)+change-prompt(search> )+disable-search+reload(indexrs search {q} --color=always --limit=500 || true)' \
  --bind 'ctrl-s:rebind(change)+change-prompt(symbol> )+disable-search+reload(indexrs symbols {q} --color=always || true)' \
  --preview 'indexrs preview {1} --line {2} 2>/dev/null || bat --style=numbers --color=always -- {1} 2>/dev/null || echo "No preview"' \
  --preview-window 'right,60%,border-left' \
  --bind 'enter:become(${EDITOR:-vim} +{2} {1})'
```
...
indexrs preview {file} --line {line}
```

The `indexrs preview` fallback uses syntect for highlighting and formats output for terminal display. It adds line numbers and highlights the target line with reverse video.
...
**With fzf.vim** -- add custom commands backed by indexrs:

```vim
" ~/.config/nvim/plugin/indexrs.vim
...
**With telescope.nvim** -- indexrs can serve as a custom picker source. A `telescope-indexrs.nvim` plugin would call indexrs as a subprocess and feed results into telescope's pipeline. The key requirement is that indexrs outputs one result per line in a parseable format, which this design satisfies.
...
1. Spawn `indexrs search` as a subprocess
...
```elisp
;; Using consult + indexrs
(defun indexrs-search ()
  "Search code using indexrs with fzf in a terminal buffer."
  (interactive)
  (let ((default-directory (project-root (project-current t))))
    (compilation-start
     (format "indexrs search '%s' --color=never"
             (read-string "Search: "))
     'grep-mode)))

;; Or use fzf.el if available
(defun indexrs-fzf ()
  "Interactive file search with indexrs + fzf."
  (interactive)
  (let ((process-environment
         (cons (format "FZF_DEFAULT_COMMAND=indexrs files") process-environment)))
...
indexrs writes results to stdout as they are found, without buffering the full result set. This means fzf starts showing results immediately. Implementation detail: use `BufWriter` with explicit `flush()` after each line, or use line-buffered stdout (default for TTY, must be forced when piping).
...
- `indexrs search`: 1000 results by default (`--limit`)
- `indexrs symbols`: 500 results by default
- `indexrs files`: no limit (file lists are typically manageable)

Users can override with `--limit=0` for unlimited results.

### 6.3 Debouncing on Reload

fzf fires `change` events on every keystroke. The `reload` action spawns a new indexrs process each time, killing the previous one. This is fine because:

1. fzf handles process lifecycle -- it kills the old process when a new reload fires
2. indexrs should handle `SIGPIPE`/broken pipe gracefully (exit immediately, no error message)
...
# indexrs should exit cleanly on SIGPIPE/broken pipe
--bind 'change:reload:indexrs search {q} --color=always --limit=500 || true'
```

No explicit debounce is needed if indexrs responds in <50ms, which is the target for indexed searches.

### 6.4 Startup Time

For the shell functions to feel instant:
- indexrs should connect to the daemon via Unix socket (not re-index on every invocation)
- If the daemon is not running, `indexrs search` should start it in the background and return results from a cold index build
- Target: <100ms to first result for common queries against a warm daemon

### 6.5 SIGPIPE Handling

When fzf kills a reload process or the user pipes output to `head`, indexrs must handle SIGPIPE without printing error messages:
...
```
indexrs files [OPTIONS]              List indexed files
indexrs search <QUERY> [OPTIONS]     Search file contents
indexrs symbols [QUERY] [OPTIONS]    Search symbols
indexrs preview <FILE> [OPTIONS]     Render file preview for fzf
