...
**Architecture:** A single new module `codec.rs` in `indexrs-core`, re-exported from `lib.rs`. Four public functions: `encode_delta_varint`, `decode_delta_varint`, `encode_positional_postings`, `decode_positional_postings`.
...
**File:** `indexrs-core/src/lib.rs` — add `pub mod codec;`

**File:** `indexrs-core/src/codec.rs` — create with module doc comment and function signatures that return empty/default values (stubs).
...
**Test:** `cargo check -p indexrs-core` — compiles with stubs.

---

## Task 2: Write failing tests for encode/decode_delta_varint

**File:** `indexrs-core/src/codec.rs` — add `#[cfg(test)] mod tests` with:
...
**Test:** `cargo test -p indexrs-core -- codec` — tests fail (stubs return empty).

---

## Task 3: Implement encode_delta_varint and decode_delta_varint

**File:** `indexrs-core/src/codec.rs`
...
**Test:** `cargo test -p indexrs-core -- codec::tests::test_roundtrip` and all delta_varint tests pass.

---

## Task 4: Write failing tests for positional postings

**File:** `indexrs-core/src/codec.rs` — add to tests:
...
**Test:** `cargo test -p indexrs-core -- codec::tests::test_positional` — tests fail.
