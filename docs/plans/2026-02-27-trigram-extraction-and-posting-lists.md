...
**Goal:** Implement trigram extraction (HHC-27) and in-memory posting lists builder (HHC-28) for the indexrs-core crate.

**Architecture:** Two new modules in indexrs-core: `trigram.rs` (extraction functions) and `posting.rs` (PostingListBuilder). Both are re-exported from lib.rs. Uses existing `Trigram` and `FileId` types from types.rs. Follows the design in `docs/design/indexing-system.md` sections 1.2.1 and Appendix A.
...
**File:** `indexrs-core/src/trigram.rs`
...
**Test:** `cargo test -p indexrs-core -- trigram` — all pass.

---

## Task 2: Create posting.rs with PostingListBuilder

**File:** `indexrs-core/src/posting.rs`
...
**Test:** `cargo test -p indexrs-core -- posting` — all pass.
