...
//! incrementally. It is stored in the `.indexrs/` directory alongside segments.
//!
...
/// File name for the checkpoint inside the `.indexrs/` directory.
...
/// Read a checkpoint from `<indexrs_dir>/checkpoint.json`.
///
...
pub fn read_checkpoint(indexrs_dir: &Path) -> Result<Option<Checkpoint>> {
    let path = indexrs_dir.join(CHECKPOINT_FILENAME);
...
/// Write a checkpoint to `<indexrs_dir>/checkpoint.json`.
///
...
pub fn write_checkpoint(indexrs_dir: &Path, checkpoint: &Checkpoint) -> Result<()> {
    fs::create_dir_all(indexrs_dir)?;

    let path = indexrs_dir.join(CHECKPOINT_FILENAME);
    let temp_path = indexrs_dir.join(format!(".checkpoint.json.tmp.{}", std::process::id()));
